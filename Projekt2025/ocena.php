...
$sql_check = "SELECT * FROM ocene WHERE id_u = $id_u AND id_f = $id_f";
$result_check = mysqli_query($link, $sql_check);
$ze_ocenil = mysqli_num_rows($result_check) > 0;

if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if ($ze_ocenil) {
        echo "<p>Ta film ste že ocenili!</p>";
    } else {
...
<?php 
if ($ze_ocenil) {
    echo '<p><strong>Ta film ste že ocenili. Vsak uporabnik lahko da samo eno oceno.</strong></p>';
} else {
    echo '<form method="post">';
    echo '<label>Ocena (1–10):</label><br>';
    echo '<input type="number" name="ocena" min="1" max="10" required><br><br>';
    echo '<label>Komentar:</label><br>';
    echo '<textarea name="komentar" rows="4" cols="40" required></textarea><br><br>';
    echo '<input type="submit" value="Oddaj oceno">';
    echo '</form>';
}
?>
</div>
